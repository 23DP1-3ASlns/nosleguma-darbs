<template>
  <div class="page-wrap">
    <div class="page-hero" style="background: linear-gradient(135deg, #1a2f4a 0%, #2d5a8e 100%)">
      <div class="container">
        <span class="section-label" style="color: rgba(255,255,255,0.6)">Informācija</span>
        <h1 class="page-title">{{ page?.title || 'Skolēniem' }}</h1>
      </div>
    </div>
...
onMounted(async () => {
  try { const res = await axios.get('/pages/skoleniem'); page.value = res.data }
