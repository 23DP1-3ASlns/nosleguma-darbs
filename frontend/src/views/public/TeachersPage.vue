<template>
  <div class="page-wrap">
    <div class="page-hero" style="background: linear-gradient(135deg, #2a1a4a 0%, #5a3d8e 100%)">
      <div class="container">
        <span class="section-label" style="color: rgba(255,255,255,0.6)">Informācija</span>
        <h1 class="page-title">{{ page?.title || 'Skolotājiem' }}</h1>
      </div>
    </div>
...
onMounted(async () => {
  try { const res = await axios.get('/pages/skolotajiem'); page.value = res.data }
